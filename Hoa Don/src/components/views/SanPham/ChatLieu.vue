      ...
      <div class="custom-modal-content quick-edit-modal shadow-lg">
        ...
        <div class="modal-body p-4 bg-white">
          <div class="mb-3">
            ...
            <input type="text" class="form-control h-38" v-model="form.maChatLieu" placeholder="Nhập mã viết tắt (Ví dụ: COTTON, KAKI...)" :disabled="modalMode === 'VIEW'" />
          </div>
          <div class="mb-3">
...
// Bổ sung maChatLieu mặc định vào đối tượng phản ứng form
const form = reactive({ id: null, maChatLieu: '', tenChatLieu: '', trangThai: 1 });

const fetchData = async () => {
  try { const res = await axios.get('http://localhost:8080/api/chat-lieu'); listData.value = res.data; } 
  catch (err) { triggerToast("Không thể tải danh sách chất liệu!", "danger"); }
};

// Cập nhật bộ lọc tìm kiếm hỗ trợ lọc cả theo Mã và Tên
const filteredData = computed(() => {
  let r = listData.value;
  if (filter.keyword.trim()) {
    const kw = filter.keyword.toLowerCase().trim();
    r = r.filter(i => 
      (i.maChatLieu && i.maChatLieu.toLowerCase().includes(kw)) ||
      (i.tenChatLieu && i.tenChatLieu.toLowerCase().includes(kw))
    );
  }
...
const openModal = (mode, item = null) => {
  ...
  // Đồng bộ bốc tách dữ liệu maChatLieu khi click con mắt xem chi tiết
  if (mode === 'VIEW' && item) {
...
const saveData = async () => {
  // Validate trống cả mã lẫn tên
  if (!form.maChatLieu.trim() || !form.tenChatLieu.trim()) {
    return triggerToast("Vui lòng nhập đầy đủ cả mã và tên chất liệu!", "danger");
  }
  try {
    if (modalMode.value === 'ADD') {
      ...
      triggerToast("Cập nhật thông tin chất liệu thành công!", "success");
    }
    closeModal();
    fetchData();
  } catch (err) { 
    console.error(err);
    triggerToast(err.response?.data || "Mã hoặc tên chất liệu bị trùng lặp!", "danger"); 
  }
};

const deleteItem = async (id) => {
  if (!confirm("Bạn có chắc muốn xóa chất liệu này?")) return;
  try { await axios.delete(`http://localhost:8080/api/chat-lieu/${id}`); triggerToast("Xóa thành công!", "success"); fetchData(); } 
  catch (err) { triggerToast("Không thể xóa chất liệu này!", "danger"); }
};
onMounted(fetchData);
</script>
