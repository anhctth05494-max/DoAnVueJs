    ...
    <div class="card border-0 shadow-sm mb-4 rounded-3 bg-white">
      <div class="card-body p-4">
        <div class="d-flex align-items-center mb-3">
          ...
          <h6 class="card-title fw-semibold mb-0 text-dark">Bộ lọc tìm kiếm Kiểu dáng</h6>
        </div>
        <div class="row g-3 align-items-end">
          <div class="col-md-5">
            ...
            <div class="input-group">
              ...
              <input v-model="filter.keyword" type="text" class="form-control rounded-end-pill border-start-0 shadow-none border-secondary-subtle" placeholder="Tìm theo mã hoặc tên kiểu dáng..." style="height: 38px; font-size: 13.5px;" />
            </div>
          </div>
          <div class="col-md-4">
          ...
          <table class="table table-hover align-middle text-nowrap" style="font-size: 0.9rem">
            <thead>
              <tr>
                ...
                <th class="py-3 px-3 border-0 fw-semibold" style="background-color: #dccbc0; color: #5a4031; width: 220px;">MÃ KIỂU DÁNG</th>
                <th class="py-3 px-3 border-0 fw-semibold" style="background-color: #dccbc0; color: #5a4031">TÊN KIỂU DÁNG</th>
            ...
            <tbody class="border-top-0 text-secondary">
              <tr v-for="(item, index) in paginatedData" :key="item.id">
                ...
                <td class="py-3 px-3 text-dark fw-bold">{{ item.maKieuDang }}</td>
                <td class="py-3 px-3 text-dark">{{ item.tenKieuDang }}</td>
              ...
              <tr v-if="filteredData.length === 0">
                <td colspan="5" class="text-center py-5 text-muted">Không tìm thấy kiểu dáng nào phù hợp.</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div class="custom-modal-content quick-edit-modal shadow-lg">
        <div class="custom-modal-header d-flex justify-content-between align-items-center">
          <h5 class="m-0 fw-bold fs-6 text-uppercase" style="color: #5a4031;">
            ...
            {{ modalMode === 'ADD' ? 'Thêm Kiểu dáng mới' : 'Cập nhật Kiểu dáng' }}
          </h5>
        ...
        <div class="modal-body p-4 bg-white">
          ...
          <div class="mb-3">
            ...
            <input type="text" class="form-control h-38" v-model="form.tenKieuDang" placeholder="Nhập tên kiểu dáng chi tiết..." />
          </div>
          <div class="mb-2">
...
const form = reactive({ id: null, maKieuDang: '', tenKieuDang: '', trangThai: 1 });

const fetchData = async () => {
  try { const res = await axios.get('http://localhost:8080/api/kieu-dang'); listData.value = res.data; } 
  catch (err) { triggerToast("Không thể tải danh sách kiểu dáng!", "danger"); }
};
...
const filteredData = computed(() => {
  ...
  if (filter.keyword.trim()) {
    ...
    r = r.filter(i => 
      (i.maKieuDang && i.maKieuDang.toLowerCase().includes(kw)) ||
      (i.tenKieuDang && i.tenKieuDang.toLowerCase().includes(kw))
    );
  }
...
const saveData = async () => {
  if (!form.maKieuDang.trim() || !form.tenKieuDang.trim()) {
    return triggerToast("Vui lòng nhập đầy đủ cả mã và tên kiểu dáng!", "danger");
  }
  try {
...
const deleteItem = async (id) => {
  if (!confirm("Bạn có chắc muốn xóa kiểu dáng này?")) return;
  try { await axios.delete(`http://localhost:8080/api/kieu-dang/${id}`); triggerToast("Xóa thành công!", "success"); fetchData(); } 
  catch (err) { triggerToast("Không thể xóa kiểu dáng này!", "danger"); }
};
onMounted(fetchData);
</script>
