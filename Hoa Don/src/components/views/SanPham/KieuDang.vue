<template>
  <div class="mx-auto my-2 page-container" style="max-width: 1200px; padding: 0 10px;">
    ...
    <ConfirmModal
      ...
      :itemName="pendingItem?.tenKieuDang"
...
const exportToExcel = () => {
  const data = filteredData.value.map((item, index) => ({
    "STT": index + 1,
    "Mã Kiểu Dáng": item.maKieuDang,
    "Tên Kiểu Dáng": item.tenKieuDang,
  ...
  XLSX.utils.book_append_sheet(wb, ws, "KieuDang");
  XLSX.writeFile(wb, "DanhSachKieuDang.xlsx");
