          ...
          <table class="table table-hover align-middle text-nowrap" style="font-size: 0.9rem">
            <thead>
              <tr>
                ...
                <th class="py-3 px-3 border-0 fw-semibold" style="background-color: #dccbc0; color: #5a4031; width: 220px;">MÃ KÍCH CỠ</th>
                <th class="py-3 px-3 border-0 fw-semibold" style="background-color: #dccbc0; color: #5a4031">SIZE KÍCH CỠ</th>
            ...
            <tbody class="border-top-0 text-secondary">
              <tr v-for="(item, index) in paginatedData" :key="item.id">
                ...
                <td class="py-3 px-3 text-dark fw-bold">{{ item.maKichCo }}</td>
                <td class="py-3 px-3 text-dark">{{ item.tenKichCo }}</td>
              ...
              <tr v-if="filteredData.length === 0">
                <td colspan="5" class="text-center py-5 text-muted">Không tìm thấy kích cỡ nào phù hợp.</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div class="custom-modal-content quick-edit-modal shadow-lg">
        <div class="custom-modal-header d-flex justify-content-between align-items-center">
          <h5 class="m-0 fw-bold fs-6 text-uppercase" style="color: #5a4031;">
            ...
            {{ modalMode === 'ADD' ? 'Thêm Kích cỡ mới' : 'Cập nhật Kích cỡ' }}
          </h5>
        ...
        <div class="modal-body p-4 bg-white">
          ...
          <div class="mb-3">
            ...
            <input type="text" class="form-control h-38" v-model="form.tenKichCo" placeholder="Nhập size kích cỡ chi tiết..." />
          </div>
          <div class="mb-2">
...
const performAction = async () => {
  try {
    if (actionType.value === 'DELETE') {
      await axios.delete(`http://localhost:8080/api/kich-co/${pendingItem.value.id}`);
      triggerToast("Xóa kích cỡ thành công!", "success");
...
const filteredData = computed(() => {
  ...
  if (filter.keyword.trim()) {
    ...
    r = r.filter(i =>
      (i.maKichCo && i.maKichCo.toLowerCase().includes(kw)) ||
      (i.tenKichCo && i.tenKichCo.toLowerCase().includes(kw))
    );
  }
