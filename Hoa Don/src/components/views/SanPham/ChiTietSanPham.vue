<template>
  <div class="container-fluid p-0">
    <div class="mb-4 d-flex justify-content-between align-items-center">
      <div>
        <h5 class="fw-bold mb-1" style="color: #5a4031">
          Chi tiết sản phẩm | {{ maSanPhamHienTai }}
        </h5>
        ...
        <div class="row g-3 align-items-end justify-content-between">
          <div class="col-md-3">
            <div class="input-group">
              <span
                class="input-group-text bg-white border-end-0 border-secondary-subtle rounded-start-pill"
                ><i class="bi bi-search"></i
              ></span>
              <input
                v-model="filter.keyword"
                class="form-control border-start-0 shadow-none border-secondary-subtle rounded-end-pill"
                placeholder="Mã SKU / Tên..."
              />
            </div>
          </div>
        ...
        <div class="row g-3 mt-3 border-top pt-3">
          <div class="col-md-2">
            <select v-model="filter.thuongHieu" class="form-select rounded-pill shadow-none">
              ...
              <option v-for="th in listThuongHieu" :key="th.id" :value="th.tenThuongHieu">
                {{ th.tenThuongHieu }}
              </option>
            </select>
          </div>
          <div class="col-md-2">
            <select v-model="filter.mauSac" class="form-select rounded-pill shadow-none">
              ...
              <option v-for="mau in listMauSac" :key="mau.id" :value="mau.tenMau">
                {{ mau.tenMau }}
              </option>
            </select>
          </div>
          <div class="col-md-2">
            <select v-model="filter.kichCo" class="form-select rounded-pill shadow-none">
              ...
              <option v-for="size in listKichCo" :key="size.id" :value="size.tenKichCo">
                {{ size.tenKichCo }}
              </option>
            </select>
          </div>
          <div class="col-md-2">
            <select v-model="filter.coAo" class="form-select rounded-pill shadow-none">
              ...
              <option v-for="co in listCoAo" :key="co.id" :value="co.tenCoAo">
                {{ co.tenCoAo }}
              </option>
            </select>
          </div>
          <div class="col-md-2">
            <select v-model="filter.tayAo" class="form-select rounded-pill shadow-none">
              ...
              <option v-for="tay in listTayAo" :key="tay.id" :value="tay.tenTayAo">
                {{ tay.tenTayAo }}
              </option>
            </select>
          </div>
          <div class="col-md-2 d-flex align-items-center">
            <div class="form-check form-switch mb-0">
              <input
                v-model="filter.conHang"
                class="form-check-input shadow-none"
                type="checkbox"
                role="switch"
                id="switchConHang"
              />
              <label class="form-check-label small text-muted" for="switchConHang"
                >Chỉ tồn kho > 0</label
              >
            </div>
          </div>
    ...
    >
      <span class="small text-muted"
        >Đã chọn <strong>{{ selectedSkus.length }}</strong> biến thể.</span
      >
      <button
  ...
  <td class="px-3 text-center">
    <span v-if="item.phanTramGiam > 0" class="badge rounded px-2 py-1 fw-normal" style="background-color: #e8f5e9; color: #2e7d32; border: 1px solid #c8e6c9;">
      <i class="bi bi-tag-fill me-1" style="font-size: 10px;"></i>{{ item.phanTramGiam }}%
    </span>
    <span v-else class="text-muted">-</span>
              ...
              <td class="px-3 text-center">
                <span
                  ...
                  :class="item.trangThai === 1 ? 'bg-primary' : 'bg-danger'"
                >
              ...
              <td class="px-3 text-center">
                <i
                  ...
                  class="bi bi-eye fs-5 text-dark"
        ...
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0" style="background-color: #f8f5f2">
            <h5 class="modal-title fw-bold" style="color: #5a4031">
              {{ modalMode === 'ADD' ? 'Thêm biến thể mới' : 'Cập nhật biến thể' }}
            </h5>
          ...
          <div class="modal-body p-4">
            ...
            <div v-if="modalMode === 'EDIT'" class="row g-3 mb-4 pb-3 border-bottom">
              <div class="col-12">
                <h6 class="fw-bold mb-0 text-muted">Thông tin gốc (Không thể sửa)</h6>
              </div>
              <div class="col-md-3">
                <label class="form-label small text-muted">Danh mục</label
                ><input :value="formInfo.tenDanhMuc" class="form-control bg-light" readonly />
              </div>
              <div class="col-md-3">
                <label class="form-label small text-muted">Thương hiệu</label
                ><input :value="formInfo.tenThuongHieu" class="form-control bg-light" readonly />
              </div>
              <div class="col-md-3">
                <label class="form-label small text-muted">Kiểu dáng</label
                ><input :value="formInfo.tenKieuDang" class="form-control bg-light" readonly />
              </div>
              <div class="col-md-3">
                <label class="form-label small text-muted">Chất liệu</label
                ><input :value="formInfo.tenChatLieu" class="form-control bg-light" readonly />
              </div>
            </div>
            ...
            <div class="row g-3">
              ...
              <div class="col-md-6">
                ...
                <input
                  :value="modalMode === 'ADD' ? generateSku : form.maSku"
                  type="text"
              ...
              <div class="col-md-3">
                ...
                <select v-model="form.idMauSac" class="form-select">
                  ...
                  <option v-for="mau in listMauSac" :key="mau.id" :value="mau.id">
                    {{ mau.tenMau }}
                  </option>
                </select>
              </div>
              <div class="col-md-3">
                ...
                <select v-model="form.idKichCo" class="form-select">
                  <option value="">-- Chọn --</option>
                  <option v-for="size in listKichCo" :key="size.id" :value="size.id">
                    {{ size.tenKichCo }}
                  </option>
                </select>
              </div>
              ...
              <div v-if="modalMode === 'EDIT'" class="col-md-3">
                ...
                <select v-model="form.idCoAo" class="form-select">
                  <option value="">-- Chọn --</option>
                  <option v-for="co in listCoAo" :key="co.id" :value="co.id">
                    {{ co.tenCoAo }}
                  </option>
                </select>
              </div>
              <div v-if="modalMode === 'EDIT'" class="col-md-3">
                ...
                <select v-model="form.idTayAo" class="form-select">
                  <option value="">-- Chọn --</option>
                  <option v-for="tay in listTayAo" :key="tay.id" :value="tay.id">
                    {{ tay.tenTayAo }}
                  </option>
                </select>
              </div>
              ...
              <div class="col-md-4">
                ...
                <input
                  :value="formatInput(form.giaNhap)"
                  @input="handleMoneyInput($event, 'giaNhap')"
                  type="text"
                  class="form-control"
                  placeholder="0"
                />
              ...
              <div class="col-md-4">
                ...
                <input
                  :value="formatInput(form.giaBan)"
                  @input="handleMoneyInput($event, 'giaBan')"
                  type="text"
                  class="form-control"
                  placeholder="0"
                />
              ...
              <div class="col-12 mt-3 border-top pt-3">
                <div class="row">
                  ...
                  <div class="col-md-4 text-center" v-if="modalMode === 'EDIT' && form.maSku">
                    ...
                    <div
                      class="p-3 border rounded shadow-sm bg-white d-inline-flex flex-column align-items-center"
                    >
                      <img
                      ...
                      <button
                        @click.prevent="downloadQR(form.maSku)"
                        class="btn btn-sm btn-outline-secondary mt-2 rounded-pill px-3 shadow-none d-flex align-items-center gap-1"
          ...
          <div class="modal-footer border-0">
            ...
            <button
              @click="validateAndTriggerConfirm"
              class="btn text-white rounded-pill px-4 shadow-none"
      ...
      <div class="confirm-overlay">
        <div class="confirm-modal-card animate__animated animate__fadeInDown animate__faster">
          ...
          <p class="confirm-message">
            Hệ thống sẽ ghi nhận những thay đổi này của biến thể sản phẩm. Bạn có chắc chắn muốn
            tiến hành lưu không?
          </p>
          <div class="confirm-actions">
            <button @click="isShowModalSuaCuaTuanAnh = false" class="btn-cancel-custom">
              Xem lại
            </button>
            <button
              @click="confirmUpdateDetails"
              class="btn-confirm-custom"
              style="background-color: #ebdcd0; color: #5a4031"
            >
              Xác nhận lưu
            </button>
          </div>
    ...
    >
      <div
        ...
        :style="
          toastType === 'success'
            ? 'background-color: #f4fbf7; border-left: 5px solid #2e7d32;'
            : 'background-color: #fff5f5; border-left: 5px solid #ef4444;'
        "
      >
        <div class="d-flex align-items-center px-3 py-2">
          ...
          <div class="d-flex flex-column py-1">
            <span
              class="fw-bold"
              :class="toastType === 'success' ? 'text-success' : 'text-danger'"
              >{{ toastType === 'success' ? 'Thành công' : 'Thất bại' }}</span
            >
...
const getPhanTramGiam = (item) => {
  // Kiểm tra xem item có đợt giảm giá không và nó còn hạn không
  // Cấu trúc giả định: item.dotGiamGia = { phanTram: 15, ngayKetThuc: '...' }
  if (item.dotGiamGia && item.dotGiamGia.phanTram > 0) {
    // Nếu muốn check thêm ngày hết hạn thì thêm vào đây
    return item.dotGiamGia.phanTram
  }
  return 0 // Trả về 0 nếu không có giảm giá
}
...
const validateAndTriggerConfirm = () => {
  ...
  if (
    parseInt(form.soLuongTon, 10) < 0 ||
    parseFloat(form.giaNhap) < 0 ||
    parseFloat(form.giaBan) <= 0
  ) {
...
const fetchChiTietMaSP = async () => {
  try {
    const res = await axios.get(
      `http://localhost:8080/api/sanpham-chitiet/by-sanpham/${route.params.id}`,
    )
...
const saveData = async () => {
  ...
  try {
    const url =
      modalMode.value === 'ADD'
        ? '/api/sanpham-chitiet/them'
        : `/api/sanpham-chitiet/cap-nhat/${form.id}`
    ...
    errorMessage.value =
      typeof errorMsgFromServer === 'string'
        ? errorMsgFromServer
