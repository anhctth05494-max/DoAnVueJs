...
// Tự động suy luận mã key
const codeKey = computed(() => {
  const map = {
    'tenThuongHieu': 'maThuongHieu', 'tenDanhMuc': 'maDanhMuc',
    'tenMau': 'maMau', 'tenChatLieu': 'maChatLieu',
    'tenKieuDang': 'maKieuDang', 'tenCoAo': 'maCoAo', 'tenTayAo': 'maTayAo'
  };
  return map[props.labelKey] || 'ma';
});
...
const sortedItems = computed(() => [...props.items].sort((a, b) => b.id - a.id));


const filteredItems = computed(() => {
  const query = searchQuery.value.toLowerCase().trim();
  if (!query) return sortedItems.value;
  return sortedItems.value.filter(item => (item[props.labelKey] || '').toLowerCase().includes(query));
});


const exactMatchExists = computed(() => {
  const query = searchQuery.value.trim().toLowerCase();
  return query && props.items.some(item => (item[props.labelKey] || '').toLowerCase() === query);
});


const toggleDropdown = () => { isOpen.value = !isOpen.value; };
const closeDropdown = () => { isOpen.value = false; };
const onInput = () => { isOpen.value = true; emit('update:modelValue', ''); emit('input'); };
const selectItem = (item) => {
  searchQuery.value = item[props.labelKey];
  emit('update:modelValue', item.id);
  isOpen.value = false;
};
const handleQuickAdd = () => { quickForm.ten = searchQuery.value; showModal.value = true; isOpen.value = false; };
const closeModal = () => { showModal.value = false; };


// 🌟 HÀM NÀY ĐÃ ĐƯỢC FIX LỖI 405 VÀ THIẾU BIẾN baseApi
const submitQuickAdd = async () => {
  if (!quickForm.ten.trim()) {
    return emit('toast', `Vui lòng nhập tên ${props.title.toLowerCase()}!`, "danger");
  }


  // Cắt bỏ "/active" để gọi API gốc
  const baseApi = props.apiEndpoint.replace('/active', '');
 
  const payload = {
    [props.labelKey]: quickForm.ten.trim(),
    [codeKey.value]: `${props.title.charAt(0).toUpperCase()}_${Date.now().toString().slice(-4)}`,
    trangThai: 1
  };


  try {
    const res = await axios.post(baseApi, payload);
    emit('toast', `Thêm nhanh ${props.title} thành công!`, "success");
   
    // Gọi refresh để load lại data
    await emit('refresh');
    ...
    const msg = err.response?.data || `Lỗi khi thêm ${props.title}`;
    emit('toast', typeof msg === 'string' ? msg : "Có lỗi xảy ra!", "danger");
  }
};
